# SQL Optima — Enterprise SQL Server → PostgreSQL Migration Platform
...
```bash
# Clone and launch the full stack (API + UI + Go migration-engine)
git clone <repo-url>
cd sqlserver_to_postgres
python start.py --all
```

Platform-specific wrappers (create `.venv` first, then delegate to `start.py`):
...
| [docs/SECURITY.md](docs/SECURITY.md) | Metadata security audit, JWT rotation, compliance notes |
