...
class L3ChunkHashValidator:
    ...
    async def validate_chunk(
        ...
        src = await self._query_source(
            source_connector, schema, table_name, pk_column,
            pk_start, pk_end, numeric_columns or [],
        )
        tgt = await self._query_target(
            target_connector, table_name, pk_column,
            pk_start, pk_end, numeric_columns or [],
        )
    ...
    async def validate_all_chunks(
        ...
        for pk_start, pk_end in chunks:
            r = await self.validate_chunk(
                ...
                schema=schema, numeric_columns=numeric_columns,
            )
            results.append(r)
            logger.debug(
                "l3_chunk_validated",
    ...
    async def _query_source(
        ...
        if pk_start is None and pk_end is None:
            sql = (
                ...
                f"MIN([{pk_col}]) AS min_pk, MAX([{pk_col}]) AS max_pk{extra} "
                f"FROM [{schema}].[{table}]"
            )
    ...
    async def _query_target(
        ...
        sum_cols = ", ".join(
            f'SUM(CAST("{c}" AS DOUBLE PRECISION)) AS "sum_{c}"'
...
class L4StatisticalSamplingValidator:
    ...
    async def validate(
        ...
        # Fetch sampled PKs from source (ORDER BY NEWID() is SQL Server syntax)
        pk_values = await self._sample_source_pks(
            source_connector, schema, table_name, pk_column, n
        )
        ...
        for pk_val in pk_values:
            src_row = await self._fetch_source_row(
                source_connector, schema, table_name, pk_column, pk_val
            )
            tgt_row = await self._fetch_target_row(
                target_connector, table_name, pk_column, pk_val
            )
            checked += 1

            if src_row is None and tgt_row is None:
                continue  # both missing — skip (shouldn't happen, but safe)
            ...
            if tgt_row is None:
                ...
                result.issues.append(
                    ValidationIssue(
                        category=ValidationCategory.QUERY_RESULT,
                        severity="error",
                        message=f"Row with {pk_column}={pk_val!r} missing in target",
                        source_value=src_row,
                        target_value=None,
                        details={"pk_value": str(pk_val)},
                    )
                )
                continue
            ...
            if src_row is None:
                ...
                result.issues.append(
                    ValidationIssue(
                        category=ValidationCategory.QUERY_RESULT,
                        severity="error",
                        message=f"Row with {pk_column}={pk_val!r} missing in source",
                        source_value=None,
            ...
            if col_diffs:
                ...
                result.issues.append(
                    ValidationIssue(
                        ...
                        severity="error",
                        message=(
                            f"Row {pk_column}={pk_val!r} has "
                            f"{len(col_diffs)} differing column(s): "
                            f"{', '.join(col_diffs)}"
                        ),
                        source_value=src_row,
                        target_value=tgt_row,
                        details={"pk_value": str(pk_val), "differing_columns": col_diffs},
                    )
                )
    ...
    async def _sample_source_pks(
        connector: Any, schema: str, table: str, pk_col: str, n: int
    ) -> list[Any] | None:
        try:
            rows = await connector.execute(
                f"SELECT TOP {n} [{pk_col}] FROM [{schema}].[{table}] ORDER BY NEWID()"
            )
    ...
    async def _fetch_source_row(
        connector: Any, schema: str, table: str, pk_col: str, pk_val: Any
    ) -> dict[str, Any] | None:
        try:
            rows = await connector.execute(
                f"SELECT * FROM [{schema}].[{table}] WHERE [{pk_col}] = ?",
                {pk_col: pk_val},
            )
            return dict(rows[0]) if rows else None
        except Exception:
    ...
    async def _fetch_target_row(
        connector: Any, table: str, pk_col: str, pk_val: Any
    ) -> dict[str, Any] | None:
        try:
            rows = await connector.execute(
...
def _compare_rows(
    ...
    for col in sorted(common):
        ...
        if sv is None and tv is None:
            continue
        if sv != tv:
            # Normalise numeric types (e.g. Decimal vs float) for comparison
            try:
